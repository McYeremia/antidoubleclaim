...
// ── Konstanta pilihan (untuk arsip detail, sinkron dengan TambahKlaimWizard) ──
const ARSIP_LABEL_KATEGORI = {
...
const arsipIsLomba = (kat) =>
  kat === "lomba_mandiri_puspresnas" || kat === "lomba_mandiri_non_puspresnas";

// ── Sub-komponen detail arsip ─────────────────────────────────────────────────
function ArsipField({ label, value }) {
  if (!value && value !== 0) return null;
  return (
    <div>
      <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">{label}</p>
...
function ArsipSectionTitle({ children }) {
...
function ArsipFileLink({ label, path }) {
  if (!path) return null;
  const filename = path.split(/[\\/]/).pop();
...
function ArsipCertPreview({ url, filename }) {
...
// ── Detail View Arsip (komprehensif) ──────────────────────────────────────────
function ArsipDetailView({ detailItem, rewards, onBack }) {
  const [pengajuan,    setPengajuan]    = useState(null);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        {/* Kolom Kanan: Data Pengajuan Lengkap + Reward */}
            ...
            {p?.estimasi_reward != null && (
              <div className="bg-amber-50 border border-amber-200 rounded-2xl px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <p className="text-[22px] font-black text-amber-700 tabular-nums">
                  {p.estimasi_reward != null ? "Rp " + Number(p.estimasi_reward).toLocaleString("id-ID") : "—"}
                </p>
              </div>
            )}
